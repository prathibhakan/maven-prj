...
public class Dashboard extends GenericFunctions {
	...
	public static boolean navigateToApplyLeave()
	{
		...
		if(status)
		{
			...
			logEvent("Pass", "DashBorad is clicked successfully");
			status=hoverAndClick(icon_ApplyLeave);
			if(status)
			{
				System.out.println("Apply Leave is clicked successfully");
				logEvent("Pass", "Apply Leave is clicked successfully");
			}
			else
			{
				System.out.println("Apply Leave is not clicked successfully");
				logEvent("Fail", "Apply Leave not is clicked successfully");
				status=false;
			}
		}
		else
		{
			...
			logEvent("Fail", "DashBorad is not clicked successfully");
			status=false;
		}
		return status;
	}
